...
var loadIntoWindow = function(win) {
  ...
  // while we're waiting for startup, let's open the onboarding flow.
  var prefBranch = prefService.getBranch('');
  var isFirstTime = prefBranch.getPrefType('services.universalSearch.isFirstTime') ?
                      prefBranch.getBoolPref('services.universalSearch.isFirstTime') : true;
  if (this.isFirstTime) {
    prefBranch.setBoolPref('services.universalSearch.isFirstTime', false);
    win.gBrowser.addTab('chrome://universalsearch/content/onboarding.html');
    // TODO: I'm not sure how window iteration works with respect to the topmost
    //       focused window, but if we need to, we can focus the new tab like this:
